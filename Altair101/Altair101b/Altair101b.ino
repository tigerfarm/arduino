...
void read_inputs_serial()
{
  ...
  else if ( data == 'm' )
  {
    ...
    if ( c == 0 )
    {
      do {
        c = serial_read();
      } while (c != 'f' && c != 'F' && c != 'h' && c != 'H' && c != 't' && c != 'T' && c != 'c' && c != 'C' && c != 27);
      if ( c != 27 ) {
        Serial.print(c);
        Serial.println(' ');
      }
    ...
    else
    {
      ...
      if ( (c == 'h' || c == 'H') )
      {
        ...
        do {
          c = serial_read();
        } while ( (c < '1' || c > '0' + NUM_HDSK_UNITS) && c != 27);
        if ( c != 27 ) {
          Serial.print(c);
          Serial.print(' ');
        }
...
void print_panel_serial(bool force)
{
  ...
  if ( force || p_cswitch != cswitch || p_dswitch != dswitch || p_abus != abus || p_dbus != dbus || p_status != status )
  {
    ...
    // Stacy, simplify for print better on my emulator.
    ...
    // Stacy, for printing to the IDE monitor.
    Serial.print(F("\r\n ------ \r\n"));
    /* Stacy, remove for printing to the IDE monitor.
